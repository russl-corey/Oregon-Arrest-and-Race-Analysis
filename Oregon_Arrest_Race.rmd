...
<!-- # Filename: Oregon_Arrest_Race.rmd -->
...
```{r Plot results map, echo=FALSE, message=FALSE, warning=FALSE}
library(rgdal)         # for readOGR(...)
library(ggplot2)
library(sf)

agencies <- read_sf("data/geo_agency/geo_agency.shp")
area <-  read_sf("data/or_counties/or_counties.shp")
...
ggplot() +
  geom_sf(data=area) +
  geom_sf(data=agencies)+
  labs(title='Agencies Used in Arrest Bias Study') +
   theme_void()
```
...
# Appendix A3 Data Distributions
```{r appendix A3, echo=FALSE, message=FALSE}
